...
class RuleParser:
    ...
    def parse_grammar_rules(filename: str) -> Dict[str, List[List[str]]]:
        ...
        返回:
            语法规则字典 {nonterminal: [[production1], [production2], ...]}
...
def load_rules_from_files(lexer_file: str, grammar_file: str) -> Tuple[List[Tuple[str, str]], Dict[str, List[List[str]]]]:
    """便捷函数：从文件加载词法规则和语法规则
    ...
    返回:
        (lexer_rules, grammar_rules) 的元组
        
    说明:
        一次性加载完整的语言定义。
    """
    lexer_rules = RuleParser.parse_lexer_rules(lexer_file)
    grammar_rules = RuleParser.parse_grammar_rules(grammar_file)
